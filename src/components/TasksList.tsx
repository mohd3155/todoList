    ...
    return (
        <div className="2xl:w-3/4 sm:w-4/5 xs:w-11/12 2xl:h-3/5 sm:h-3/5 xs:h-3/4 sm:flex 2xl:flex-row xs:flex-col 2xl:justify-center 2xl:items-center xs:mt-5">
            ...
            <div className="2xl:w-1/12 sm:w-2/4 xs:w-full 2xl:h-1/6 2xl:border-b-2 2xl:border-r-2 xs:border-r-0 2xl:border-white 2xl:pr-2 ">
                ...
                <select data-testid="filter" onChange={(e) => handleFilter(e.target.value)} className="2xl:w-full xs:w-2/4 xs:bg-[#1e2732] xs:text-white">
            ...
            {tasks.map((task) => <div data-testid="tasksList" key={task.id} className="2xl:w-full xs:w-full 2xl:h-1/6 xs:h-full xs:flex xs:flex-row xs:justify-between xs:items-center xs:border-b-2 xs:border-white xs:mt-3 xs:pb-3">
                <div data-testid={task.status === "Active" ? "Active-tasksList-tasks" : "Completed-tasksList-tasks"} className="2xl:w-1/4 xs:w-1/5 2xl:h-full xs:flex xs:justify-center xs:items-center  "><span className="2xl:w-full xs:max-w-full xs:text-center lg:text-lg xs:text-white xs:text-xs xs:truncate 2xl:font-bold">{task.task}</span></div>
                <div data-testid={task.status === "Active" ? "Active-tasksList-desc" : "Completed-tasksList-desc"} className="2xl:w-1/4 xs:w-1/5 2xl:h-full xs:flex xs:justify-center xs:items-center break-words"><span className="2xl:w-full xs:max-w-full xs:text-xs lg:text-lg xs:text-center  line-clamp-3  xs:text-white">{task.description}</span></div>
                <div data-testid={task.status === "Active" ? "Active-tasksList-status" : "Completed-tasksList-status"} className="2xl:w-1/4 xs:w-1/5 2xl:h-full xs:flex xs:justify-center xs:items-center "><span className={`2xl:w-full xs:w-full xs:text-center xs:text-xs lg:text-lg lg:font-bold xs:truncate ${task.status === "Active" ? "text-red-600": "text-green-600"}`}>{task.status}</span></div>
                <div className="2xl:w-1/4 xs:w-1/5 2xl:h-full xs:flex xs:justify-center xs:items-center ">
                    <select data-testid = "change-status" className="lg:w-2/4 xs:w-full xs:bg-[#1e2732]  xs:text-sm lg:text-lg xs:text-white"  onChange={(e) => changeStatus(e.target.value,task.id)}>
