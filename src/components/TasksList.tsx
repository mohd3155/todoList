    ...
    return (
        <div className="2xl:w-3/4 sm:w-4/5 xs:w-11/12 2xl:h-3/5 sm:h-3/5 xs:h-3/4 sm:flex 2xl:flex-row xs:flex-col 2xl:justify-center 2xl:items-center xs:mt-5">
            ...
            {tasks.map((task) => <div data-testid="tasksList" key={task.id} className="2xl:w-full xs:w-full 2xl:h-1/6 xs:h-full xs:flex xs:flex-row xs:justify-between xs:items-center xs:border-b-2 xs:border-white xs:mt-3 xs:pb-3">
                <div data-testid={getStatus(filter) + "-tasksList-tasks"} className="2xl:max-w-1/4 xs:w-1/5 2xl:h-full xs:flex xs:justify-center xs:items-center  "><span className="2xl:w-full xs:max-w-full xs:text-center lg:text-lg xs:text-white xs:text-xs xs:truncate 2xl:font-bold">{task.task}</span></div>
                <div data-testid={getStatus(filter) + "-tasksList-desc" } className="2xl:max-w-1/4 xs:w-1/5 2xl:h-full xs:flex xs:justify-center xs:items-center break-words"><span className="2xl:w-full xs:max-w-full xs:text-xs lg:text-lg xs:text-center  line-clamp-3  xs:text-white">{task.description}</span></div>
                ...
                <div className="2xl:w-1/4 xs:w-1/5 2xl:h-full lg:h-full xs:flex xs:justify-center xs:items-center"><button className="2xl:w-2/4 lg:w-2/4 sm:w-3/4 xs:w-full xs:h-full lg:h-full h-3/5 rounded-2xl text-[#1e2732] 2xl:text-lg xs:text-xs font-semibold bg-white hover:bg-[#1e2732] hover:text-white hover:border hover:border-white transition duration-200 delay-100" onClick={() => handleDelete(task.id)}>Delete</button></div>
            </div> )}
            </div>
            </div>
        </div>
